...
io.on("connection", (socket: Socket<IncommingMessages, OutgoingMessages>) => {
   ...
   socket.on("createRoom/in", (roomNumber, chosenGeneralTopic, chosenPackname, hostName) => {
      if (!Array.from(socket.rooms.values()).includes(roomNumber)) {
         if (!Object.keys(roomData).includes(roomNumber)) {
            roomData[roomNumber] = {generalTopic: chosenGeneralTopic, packname: chosenPackname, hostName, gameStarted: false };
            ...
            if (room.players && room.players.length > 0) {
               if (room.players.length < 4) {
                  if (!room.players.find(({userName: joinedName}) => joinedName === userName)) {
                     room.players = [...room.players, {userName, socketId, points: 0}];
                  }
               } else {
